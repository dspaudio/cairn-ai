...
1. Every agent must read the project-root `MEMORY.md` for domain knowledge and repository policy before review work, then read the plan slice and memory inputs.
2. Check changed files, tool readiness evidence, and verification evidence.
3. Delegate independent read-only review to `explorer` when it materially improves speed or quality; use `worker` only for scoped verification reruns or QA artifacts. Tell every delegated agent to read the project-root `MEMORY.md` before work.
