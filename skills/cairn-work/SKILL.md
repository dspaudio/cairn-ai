...
2. 계획에 기록된 `docs/model-guidance/*.md` 입력을 읽습니다.
3. 첫 번째 미완료 모듈 조각을 선택합니다.
4. 계획의 선택 경로를 확인합니다.
5. 빠른 경로이면 `planner → builder` 순서를 유지하고 바로 `builder`에게 위임합니다.
6. 전체 경로이면 `architect → planner → reviewer → builder → reviewer` 순서를 유지합니다. 구현 전 `reviewer`는 계획 공백을 확인하고, 구현 후 `reviewer`는 증거와 회귀를 확인합니다.
7. 정확한 파일, 제약, 모델별 보정 규칙을 포함해 `builder` 또는 `worker`에게 구현을 위임합니다.
8. `builder`는 변경 파일, 모듈 수용 명령과 결과, 표면 통합 명령 또는 QA 산출물, 정리 내역을 반환해야 합니다.
9. 두 검증을 직접 다시 실행합니다.
10. 조각 완료 표시 전에 계획 파일에 증거를 기록합니다.
...
CONTEXT: docs/plan/<topic>.md 조각, 관련 docs/memory 노트, 적용 모델 지침, 정확한 파일, 검증 명령.
```
