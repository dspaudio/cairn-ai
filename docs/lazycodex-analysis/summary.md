...
LazyCodex는 여러 모델 계열별 지침을 세밀하게 나눕니다. Cairn 0.1.0은 범위를 좁혀 Claude 계열과 Codex 계열만 적용합니다.

- Claude 계열: 긴 컨텍스트 해석, 정책 판단, 아키텍처 검토, 계획 일관성 리뷰에 우선 배정합니다.
- Codex 계열: 작은 구현 조각, 명확한 파일 편집, 명령 기반 검증, 반복 가능한 작업 실행에 우선 배정합니다.
- Kimi, Gemini 등 다른 모델 계열 지침은 0.1.0 범위에 포함하지 않습니다.
