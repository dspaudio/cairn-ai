...
12. Run focused tests, then one final full repository check through `goal verify -- ...` so exact argv, tool exit code, bounded output, and watched-workspace fingerprint are recorded together. Any relevant later mutation makes affected evidence stale.
